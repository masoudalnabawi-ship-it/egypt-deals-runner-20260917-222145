...
class Dream2000Connector(StoreConnector):
    ...
    @staticmethod
    def _image_url(product, variant=None):
        variant = variant or {}
        ...
        if isinstance(featured, dict):
            ...
            if src:
                return str(src)
        elif isinstance(featured, str) and featured:
            return featured
        ...
        for variant in variants:
            ...
            deals.append(
                Deal(
                    ...
                    old_price=old_price,
                    url=url,
                    image_url=self._image_url(product, variant),
                )
            )
    ...
    async def fetch_observations(self):
        ...
        for page in range(1, 9):
            ...
            for product in products:
                for deal in self._product_deals(product, discounts_only=False):
                    key = (
                        deal.title.lower().strip(),
                        deal.current_price,
                        deal.url,
