...
class AmazonConnector(StoreConnector):
    ...
    def _parse_search_cards(self, soup):
        ...
        for card in cards:
            ...
            image_el = card.select_one("img.s-image, img[data-image-latency]")
            image_url = None
            if image_el:
                image_url = (
                    image_el.get("src")
    ...
    async def recheck_exact(self, deal):
        """Read the exact Amazon product page once for Flash Review."""
        url = str(getattr(deal, "url", "") or "").strip()
        if not url:
            return None
