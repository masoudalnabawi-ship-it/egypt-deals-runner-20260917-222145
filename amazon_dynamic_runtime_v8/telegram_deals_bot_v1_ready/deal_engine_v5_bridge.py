...
def evaluate_record_v5(rec, current_price):
    ...
    result = evaluate_deal(
        ...
        member_only=bool(rec.get("member_only", False)),
        promo_scope=str(rec.get("promo_scope", "") or ""),
        account_specific=bool(rec.get("account_specific", False)),
    )
    ...
    result["promo_scope"] = str(rec.get("promo_scope", "") or "")
    result["account_specific"] = bool(rec.get("account_specific", False))
    result["bank_only"] = bool(rec.get("bank_only", False))
    result["applies_to_all"] = bool(rec.get("applies_to_all", False))
    result["general_50_plus"] = bool(rec.get("general_50_plus", False))
    result["bank_offers"] = rec.get("bank_offers", []) or []
    result["member_offers"] = rec.get("member_offers", []) or []
    result["account_offers"] = rec.get("account_offers", []) or []
    result["public_offers"] = rec.get("public_offers", []) or []
