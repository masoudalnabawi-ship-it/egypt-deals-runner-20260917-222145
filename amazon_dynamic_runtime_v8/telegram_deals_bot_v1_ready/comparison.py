...
def _product_key_base(deal: Deal) -> str:
...
def product_key(deal: Deal) -> str:
    base = _product_key_base(deal)
    surface = str(getattr(deal, "surface", "") or "").lower()
    condition = str(getattr(deal, "condition", "") or "new").lower()

    if surface == "amazon_used" or condition != "new":
        context = "used:" + condition
    elif surface == "amazon_now":
        context = "amazon_now"
    elif surface == "noon_minutes":
        context = "noon_minutes"
    else:
        context = "retail_new"
